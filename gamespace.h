...
class GameSpace : public QGraphicsView
{
	...
	public: // Methods
		...
		// Modifiers
		void initialize();
		void startLevel(bool maintainGame, int level);
		void startLevel(bool maintainGame);
	...
	private: // Properties
		...
		QGraphicsPixmapItem* backgroundPicItem_;
			/** Vector of pointers to the pictures used as the background */
			std::vector<QPixmap*> backgroundPics_;
		...
		bool gameOverFlag_;
		/** Indicates which level you're currently on */
		int level_;
};
